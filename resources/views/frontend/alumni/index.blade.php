    ...
    <section class="flex flex-col items-center py-16 lg:py-[96px] px-6 lg:px-[64px] w-full bg-[#FBF8FC] relative">
        ...
        <div class="flex flex-col w-full max-w-[1152px] gap-12 z-10">
            ...
            <div class="flex flex-col gap-6">
                @foreach($featuredAlumni as $index => $alumni)
                    @php
                        $isReversed = $index % 2 !== 0;
                    @endphp
                    <div class="flex flex-col {{ $isReversed ? 'lg:flex-row-reverse' : 'lg:flex-row' }} border border-[#E4E1E5] bg-[#FBF8FC] overflow-hidden reveal-on-scroll reveal-up">
                        
                        <!-- Photo Side -->
                        <div class="relative w-full lg:w-1/2 min-h-[350px] lg:min-h-[522px] overflow-hidden">
                            @if($alumni->photo)
                                <img src="{{ Storage::url($alumni->photo) }}" alt="{{ $alumni->name }}" class="w-full h-full object-cover hover:scale-105 transition-transform duration-700" loading="lazy">
                            @else
                        ...
                        <div class="flex flex-col justify-between p-8 lg:p-[32px] w-full lg:w-1/2">
                            <div class="flex flex-col gap-1">
                                <!-- Year -->
                                <span class="font-sans font-bold text-[12px] tracking-[1.2px] text-[#5F5E5E] uppercase mb-1">Angkatan {{ $alumni->graduation_year }}</span>
                                
                                <!-- Name -->
                                <h3 class="font-heading font-bold text-[24px] leading-[31px] text-[#1B1B1E] mt-1">
                                    <a href="{{ route('alumni.show', $alumni->slug) }}" class="hover:text-[#B70011] transition-colors">{{ $alumni->name }}</a>
                                </h3>
                                
                                <!-- Position -->
                                <div class="pb-5">
                                    <p class="font-sans font-bold text-[16px] leading-[24px] text-[#B70011]">
                                        {{ $alumni->current_occupation ?: 'Alumni' }}@if($alumni->current_company), {{ $alumni->current_company }}@endif
                                    </p>
                                </div>
                                ...
                                <div class="border-l-2 border-[#B70011] pl-[18px]">
                                    <p class="font-sans italic text-[18px] leading-[29px] text-[#5C403C]">
                                        "{{ Str::limit(strip_tags($alumni->success_story), 200) }}"
                                    </p>
                                </div>
                                @endif
                            </div>
                            ...
                            <div class="flex items-center gap-2 mt-6 pt-6">
                                ...
                                <span class="font-sans font-bold text-[12px] tracking-[1.2px] text-[#5F5E5E] uppercase">{{ $alumni->city ?: 'Indonesia' }}</span>
                            </div>
                        </div>
                    </div>
                @endforeach
    ...
    <section class="flex flex-col items-center bg-[#F5F3F6] py-16 lg:py-[96px] px-6 lg:px-[64px] w-full border-t border-[#E4E1E5]">
        <div class="flex flex-col w-full max-w-[1152px] z-10">
            ...
            <div class="grid grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6 w-full">
                @foreach($otherAlumni as $alumni)
                <a href="{{ route('alumni.show', $alumni->slug) }}" class="group relative flex flex-col items-center bg-[#FBF8FC] border border-[#E4E1E5] overflow-hidden hover:border-[#B70011] transition-colors reveal-on-scroll reveal-up">
                    <!-- Square Photo -->
                    <div class="w-full aspect-square relative overflow-hidden bg-gray-100">
                        @if($alumni->photo)
                            <img src="{{ Storage::url($alumni->photo) }}" alt="{{ $alumni->name }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-500" loading="lazy">
                        @else
                    ...
                    <div class="p-4 w-full text-center">
                        <h3 class="font-heading font-bold text-[16px] text-[#1B1B1E] group-hover:text-[#B70011] transition-colors line-clamp-1">{{ $alumni->name }}</h3>
                        <p class="font-sans text-[13px] text-[#5F5E5E] mt-1">Angkatan {{ $alumni->graduation_year }}</p>
                        <p class="font-sans font-medium text-[12px] text-[#B70011] uppercase tracking-wide mt-1 line-clamp-1">{{ $alumni->current_occupation ?: 'Alumni' }}</p>
                    </div>
                </a>
                @endforeach
            </div>
