        ...
        <x-frontend.layout.container class="relative z-10">
            ...
            <div class="text-center max-w-4xl mx-auto mb-16 reveal-on-scroll reveal-up">
                ...
                <div class="flex flex-col sm:flex-row items-center justify-center gap-4">
                    ...
                    <x-frontend.ui.button href="{{ route('about') }}" variant="outline" size="lg" class="w-full sm:w-auto bg-white hover:bg-charcoal-50 border-charcoal-200">
                        Profil Jurusan
                    </x-frontend.ui.button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 lg:grid-cols-4 gap-4 lg:gap-6 auto-rows-[240px] lg:auto-rows-[280px]">
                ...
                <a href="{{ route('gallery.index') }}" class="group relative md:col-span-2 md:row-span-2 rounded-3xl overflow-hidden shadow-sm hover:shadow-2xl transition-all duration-500 reveal-on-scroll reveal-up">
                    <img src="{{ $settings->get('hero_image') ? Storage::url($settings->get('hero_image')) : 'https://images.unsplash.com/photo-1619642751034-765dfdf7c58e?q=80&w=1200&auto=format&fit=crop' }}" alt="Praktik" class="absolute inset-0 w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                ...
                <!-- BENTO 2: Stat - Alumni -->
                <a href="{{ route('alumni.index') }}" class="bg-charcoal-950 rounded-3xl p-6 lg:p-8 flex flex-col justify-between shadow-sm border border-charcoal-800 relative overflow-hidden group hover:border-charcoal-600 transition-colors reveal-on-scroll reveal-up delay-100">
                    ...
                    <div>
                        <p class="text-4xl lg:text-5xl font-black text-white tracking-tight">{{ $alumniCount ?: 1200 }}+</p>
                ...
                <a href="{{ route('partnership.index') }}" class="bg-primary-600 rounded-3xl p-6 lg:p-8 flex flex-col justify-between shadow-lg shadow-primary-600/30 text-white hover:bg-primary-700 transition-colors group reveal-on-scroll reveal-up delay-200">
                    ...
                    <div>
                        <p class="text-4xl lg:text-5xl font-black tracking-tight">{{ $partnerCount ?: 10 }}+</p>
                    ...
                    <div class="flex-grow flex flex-col justify-center gap-4">
                        ...
                        <a href="{{ route('announcements.show', $agenda->slug) }}" class="group flex items-center gap-4 p-2 -m-2 rounded-xl hover:bg-charcoal-50 transition-colors">
                            <div class="w-14 h-14 rounded-2xl bg-charcoal-100 flex flex-col items-center justify-center border border-charcoal-200 group-hover:bg-primary-50 group-hover:border-primary-200 group-hover:text-primary-600 transition-colors shrink-0">
                                <span class="text-[10px] font-bold text-charcoal-500 uppercase">{{ $agenda->created_at->translatedFormat('M') }}</span>
                                <span class="text-lg font-black text-charcoal-900">{{ $agenda->created_at->format('d') }}</span>
                            </div>
                            <div class="flex-grow">
                                <h4 class="text-sm font-bold text-charcoal-900 group-hover:text-primary-600 transition-colors line-clamp-1 mb-0.5">{{ $agenda->title }}</h4>
                                <p class="text-xs text-charcoal-500 font-medium line-clamp-1">{{ Str::limit(strip_tags($agenda->content), 60) }}</p>
                            </div>
                        </a>
                        @empty
                ...
                <div class="grid grid-cols-1 md:grid-cols-12">
                    <div class="md:col-span-4 lg:col-span-5 relative min-h-[300px]">
                        @if($headOfDepartment->photo)
                            <img src="{{ Storage::url($headOfDepartment->photo) }}" alt="{{ $headOfDepartment->name }}" class="absolute inset-0 w-full h-full object-cover object-top grayscale-[30%]">
                        @else
                    ...
                    <div class="md:col-span-8 lg:col-span-7 p-8 lg:p-12 flex flex-col justify-center">
                        ...
                        <h2 class="text-3xl lg:text-4xl font-black text-white tracking-tight uppercase leading-tight mb-2">{{ $headOfDepartment->name }}</h2>
                        @if($headOfDepartment->specialization)
                            <p class="text-charcoal-400 font-bold uppercase tracking-widest text-xs mb-8">{{ $headOfDepartment->specialization }}</p>
                        @endif
                        ...
                        <div>
                            <x-frontend.ui.button href="{{ route('academic.teachers') }}" variant="primary" class="bg-white text-charcoal-950 hover:bg-charcoal-200 shadow-none border-none">
    ...
    <section class="py-20 lg:py-28 bg-charcoal-50 border-b border-charcoal-100">
        <x-frontend.layout.container>
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-8 lg:gap-10">
                ...
                <div class="bg-white rounded-3xl overflow-hidden border border-charcoal-200 hover:border-primary-200 hover:shadow-xl transition-all duration-300 flex flex-col reveal-on-scroll reveal-up delay-{{ $loop->iteration * 100 }}">
                    <div class="w-full aspect-[16/9] relative overflow-hidden bg-charcoal-100 border-b border-charcoal-100">
                        <img src="{{ $program->thumbnail ? Storage::url($program->thumbnail) : 'https://images.unsplash.com/photo-1517524008697-84bbe3c3fd98?q=80&w=600&auto=format&fit=crop' }}" alt="{{ $program->name }}" class="absolute inset-0 w-full h-full object-cover">
                    </div>
                    <div class="p-8 flex flex-col flex-grow justify-between">
                        <div>
                            <h3 class="text-2xl font-black uppercase text-charcoal-950 mb-3 tracking-tight">{{ $program->name }}</h3>
                        ...
                        <a href="{{ route('academic.programs') }}#{{ $program->slug }}" class="inline-flex items-center justify-center px-5 py-2.5 bg-charcoal-50 text-charcoal-900 text-[10px] font-black uppercase tracking-widest rounded-xl border border-charcoal-200 hover:bg-primary-50 hover:text-primary-600 hover:border-primary-200 transition-colors w-max">
                            Pelajari Kompetensi
            ...
            @else
            <x-frontend.ui.empty-state title="Program Keahlian" message="Informasi program sedang diperbarui." icon="academic-cap" />
            @endif
        </x-frontend.layout.container>
    </section>
        ...
        <x-frontend.layout.container class="relative z-10">
            ...
            <div class="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6 auto-rows-[300px] lg:auto-rows-[400px]">
                @foreach($facilities as $index => $facility)
                    ...
                    @endphp
                    <a href="{{ route('academic.facilities') }}" class="{{ $colSpan }} bg-charcoal-900 rounded-3xl relative overflow-hidden group shadow-sm flex flex-col justify-end reveal-on-scroll reveal-up delay-{{ $index * 100 }}">
                        <img src="{{ $facility->image ? Storage::url($facility->image) : 'https://images.unsplash.com/photo-1581092580497-e0d23cbdf1dc?q=80&w=800&auto=format&fit=crop' }}" alt="{{ $facility->name }}" class="absolute inset-0 w-full h-full object-cover opacity-60 group-hover:opacity-80 group-hover:scale-105 transition-all duration-700" loading="lazy">
            ...
            @else
            <x-frontend.ui.empty-state title="Data Fasilitas" message="Belum ada data fasilitas yang ditambahkan." icon="document" />
            @endif
        </x-frontend.layout.container>
    </section>
                ...
                @if($partners->isNotEmpty())
                    <div class="grid grid-cols-2 md:grid-cols-4 lg:grid-cols-8 gap-4 lg:gap-6">
                        @foreach($partners as $partner)
                            <a href="{{ route('partnership.show', $partner->slug) }}" class="bg-charcoal-900 border border-charcoal-800 rounded-2xl aspect-square flex flex-col items-center justify-center p-4 hover:border-primary-500 hover:bg-charcoal-800 transition-all group overflow-hidden">
                                <img src="{{ $partner->logo ? Storage::url($partner->logo) : 'https://ui-avatars.com/api/?name='.urlencode($partner->name).'&background=1e293b&color=fff&size=128&bold=true' }}" alt="{{ $partner->name }}" class="max-h-12 max-w-full object-contain grayscale opacity-60 group-hover:grayscale-0 group-hover:opacity-100 transition-all">
                            </a>
                        @endforeach
                    </div>
                    <div class="text-center mt-8">
                        <a href="{{ route('partnership.index') }}" class="text-[10px] font-black uppercase tracking-widest text-primary-500 hover:text-primary-400 flex items-center justify-center gap-2">
            ...
            <div class="reveal-on-scroll reveal-up">
                <div class="flex flex-col md:flex-row md:items-end justify-between mb-10">
                    ...
                    <div class="mt-4 md:mt-0">
                        <x-frontend.ui.button href="{{ route('jobs.index') }}" variant="primary" class="bg-white text-charcoal-950 border-none hover:bg-charcoal-200">
                            Jelajahi Lowongan
                        </x-frontend.ui.button>
                    </div>
                </div>
                ...
                @if($jobVacancies->isNotEmpty())
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-4 lg:gap-6">
                        @foreach($jobVacancies as $job)
                            <a href="{{ route('jobs.index') }}" class="bg-charcoal-900 rounded-3xl p-6 lg:p-8 border border-charcoal-800 hover:border-primary-500 hover:-translate-y-1 transition-all group">
                                <div class="flex justify-between items-start mb-6">
                                    <div class="w-12 h-12 bg-white rounded-xl flex items-center justify-center overflow-hidden shrink-0 border border-charcoal-200">
                                        <img src="{{ ($job->industryPartner && $job->industryPartner->logo) ? Storage::url($job->industryPartner->logo) : 'https://ui-avatars.com/api/?name='.urlencode($job->industryPartner->name ?? 'Company').'&background=1e293b&color=fff&size=128&bold=true' }}" alt="Logo" class="w-full h-full object-contain p-2">
                                    </div>
                                ...
                                <h3 class="text-xl font-bold text-white mb-2 leading-tight group-hover:text-primary-400 transition-colors">{{ $job->title }}</h3>
                                <p class="text-sm font-medium text-charcoal-400 mb-6">{{ $job->industryPartner->name ?? 'Perusahaan Mitra' }}</p>
                                ...
                                <div class="flex items-center text-[10px] font-black uppercase tracking-widest text-charcoal-500 border-t border-charcoal-800 pt-4">
                                    Berlaku s.d: {{ $job->closing_date ? \Carbon\Carbon::parse($job->closing_date)->translatedFormat('d M Y') : 'Tidak ditentukan' }}
                                </div>
                            </a>
                        @endforeach
                    </div>
    ...
    <section class="py-20 lg:py-28 bg-charcoal-50 border-b border-charcoal-100">
        <x-frontend.layout.container>
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 reveal-on-scroll reveal-up">
                ...
                <div class="mt-6 md:mt-0">
                    <x-frontend.ui.button href="{{ route('alumni.index') }}" variant="outline" class="border-charcoal-200">
                        Cari Data Alumni
                    </x-frontend.ui.button>
                </div>
            </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4 lg:gap-6">
                ...
                <div class="bg-white rounded-3xl p-6 border border-charcoal-200 shadow-sm flex items-start gap-5 reveal-on-scroll reveal-up delay-{{ ($index % 3) * 100 }}">
                    ...
                    <div>
                        <h4 class="text-base font-bold text-charcoal-900 mb-1 leading-tight">{{ $alumni->name }}</h4>
                        <div class="text-[10px] font-black uppercase tracking-widest text-primary-600 mb-2">Lulusan {{ $alumni->graduation_year }}</div>
                        @if($alumni->current_job || $alumni->current_company)
                            <p class="text-xs text-charcoal-600 font-medium leading-relaxed">
                                {{ $alumni->current_job }}{{ $alumni->current_job && $alumni->current_company ? ' di ' : '' }}<span class="font-bold text-charcoal-800">{{ $alumni->current_company }}</span>
                            </p>
                        @endif
                    </div>
                </div>
            ...
            @else
            <x-frontend.ui.empty-state title="Data Alumni" message="Belum ada profil alumni yang ditambahkan." icon="users" />
            @endif
        </x-frontend.layout.container>
    </section>
        ...
        <x-frontend.layout.container>
            <div class="flex flex-col md:flex-row md:items-end justify-between mb-12 reveal-on-scroll reveal-up">
                ...
                <div class="mt-6 md:mt-0 flex gap-3">
                    <x-frontend.ui.button href="{{ route('gallery.index') }}" variant="outline" class="border-charcoal-200">
                        Galeri
                    </x-frontend.ui.button>
                    <x-frontend.ui.button href="{{ route('news.index') }}" variant="primary">
                        Semua Berita
                    </x-frontend.ui.button>
                </div>
            </div>
            ...
            <div class="mb-12">
                ...
                <div class="grid grid-cols-1 md:grid-cols-3 gap-6 lg:gap-8">
                    ...
                    <a href="{{ route('news.show', $news->slug) }}" class="group bg-white rounded-3xl overflow-hidden shadow-sm border border-charcoal-200 hover:shadow-xl hover:-translate-y-1 hover:border-charcoal-300 transition-all duration-300 flex flex-col reveal-on-scroll reveal-up delay-{{ $loop->iteration * 100 }}">
            ...
            <div class="reveal-on-scroll reveal-up">
                ...
                <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
                    ...
                    <a href="{{ route('gallery.show', $gallery->slug) }}" class="aspect-square rounded-3xl overflow-hidden bg-charcoal-100 relative group block shadow-sm border border-charcoal-200">
                        <img src="{{ $gallery->cover_image ? Storage::url($gallery->cover_image) : 'https://images.unsplash.com/photo-1517524008697-84bbe3c3fd98?q=80&w=600&auto=format&fit=crop' }}" alt="{{ $gallery->title }}" class="w-full h-full object-cover group-hover:scale-105 transition-transform duration-700">
                        <div class="absolute inset-0 bg-charcoal-950/60 opacity-0 group-hover:opacity-100 transition-opacity duration-300 flex items-center justify-center text-center p-4">
                            <span class="text-white text-xs font-bold leading-tight">{{ $gallery->title }}</span>
                        </div>
                    </a>
                    @endforeach
                </div>
