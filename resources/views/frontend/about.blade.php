<x-layouts.app title="Profil Akademik">
    ...
    <div class="bg-blue-600 py-16 lg:py-24">
        <div class="max-w-screen-xl mx-auto px-4 text-center">
            ...
            <p class="text-blue-100 text-lg max-w-2xl mx-auto">Mengenal lebih dekat fasilitas, tenaga pengajar, dan kompetensi keahlian Teknik Otomotif.</p>
        </div>
    </div>
            ...
            <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
                @forelse($teachers as $teacher)
                    <div class="bg-white border border-slate-100 rounded-2xl overflow-hidden shadow-sm hover:shadow-md transition-shadow text-center p-6 group">
                        <div class="w-24 h-24 mx-auto bg-slate-200 rounded-full overflow-hidden mb-4 border-4 border-slate-50 group-hover:border-blue-100 transition-colors">
                            @if($teacher->photo)
                        ...
                        <p class="text-blue-600 text-sm font-medium mb-3">{{ $teacher->position ?? 'Guru Kejuruan' }}</p>
                        @if($teacher->nip)
