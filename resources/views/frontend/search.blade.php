<x-layouts.app title="Pencarian Global" robots="noindex, follow">
    <div class="bg-charcoal-50 pt-10 pb-14 lg:pt-14 lg:pb-20 border-b border-charcoal-200">
        <div class="max-w-screen-xl mx-auto px-4 text-center reveal-on-scroll reveal-up">
            ...
            <p class="text-charcoal-600 text-lg max-w-2xl mx-auto">Temukan informasi seputar jurusan {{ $settings->get('site_name', 'Teknik Sepeda Motor') }} ({{ $settings->get('site_short_name', 'TSM') }}).</p>
            <form action="{{ route('search') }}" method="GET" class="mt-8 max-w-xl mx-auto relative reveal-on-scroll reveal-up delay-100">
                <input type="text" name="q" value="{{ $q }}" placeholder="Ketik kata kunci pencarian..." class="w-full px-6 py-4 rounded-sm sm:rounded-[2px] text-charcoal-800 bg-white border border-charcoal-200 focus:outline-none focus:ring-2 focus:ring-primary-600 shadow-sm text-lg">
                <button type="submit" class="absolute right-2 top-2 bottom-2 bg-primary-600 text-white px-6 rounded-sm sm:rounded-[2px] font-semibold hover:bg-primary-700 focus:outline-none focus:ring-2 focus:ring-primary-600 transition">Cari</button>
            </form>
        </div>
    </div>
    ...
    <section class="bg-white min-h-[50vh] pt-12 pb-20 lg:pt-16 lg:pb-24">
        <div class="max-w-screen-md mx-auto px-4 reveal-on-scroll reveal-up">
            ...
            @elseif($totalResults === 0)
                <div class="text-center py-20 bg-charcoal-50 rounded-sm sm:rounded-none shadow-sm border border-charcoal-200">
            ...
            @else
                ...
                <div class="space-y-12">
                    @foreach($results as $groupName => $items)
                        @if(count($items) > 0)
                            <div class="bg-white rounded-sm sm:rounded-none shadow-sm border border-charcoal-200 overflow-hidden reveal-on-scroll reveal-up delay-{{ $loop->iteration * 100 }}">
                                <div class="bg-charcoal-50 px-6 py-4 border-b border-charcoal-200">
                                    <h3 class="font-bold text-charcoal-900">{{ $groupName }} <span class="ml-2 bg-primary-100 text-primary-700 text-xs py-1 px-2 rounded-sm">{{ count($items) }}</span></h3>
                                </div>
                                    ...
                                    @foreach($items as $item)
                                        <a href="{{ $item->url }}" class="block p-6 hover:bg-charcoal-50 transition-colors group">
                                            <div class="flex justify-between items-start mb-2">
                                                ...
                                                @if($item->date)
                                                    <span class="text-xs font-medium text-charcoal-500 bg-charcoal-100 px-2 py-1 rounded-sm">{{ $item->date }}</span>
                                                @endif
                                            </div>
                                            @if($item->excerpt)
