...
>
    ...
    <div class="bg-charcoal-50 pt-10 pb-14 lg:pt-14 lg:pb-20 relative overflow-hidden border-b border-charcoal-200">
        ...
        <x-frontend.layout.container class="relative z-10 text-center max-w-4xl mx-auto reveal-on-scroll reveal-up">
            <span class="inline-block py-1 px-4 bg-primary-100 text-primary-700 rounded-sm text-xs font-bold tracking-widest uppercase mb-6 border border-primary-200 shadow-sm">
                Album Dokumentasi
            </span>
            ...
            <div class="flex flex-wrap items-center justify-center text-charcoal-600 text-sm gap-4 md:gap-6 mt-6 font-semibold">
                @if($album->event_date)
                <span class="flex items-center px-4 py-2 bg-white rounded-sm border border-charcoal-200 shadow-sm">
    ...
    @keydown.escape.window="isOpen = false">
        <x-frontend.layout.container>
            ...
            @if($album->description)
                <div class="max-w-4xl mx-auto mb-16">
                    <div class="bg-charcoal-50 p-8 md:p-10 rounded-sm sm:rounded-none shadow-sm border border-charcoal-100 text-center relative overflow-hidden group">
            ...
            @if($album->items->isNotEmpty())
                <div class="columns-1 sm:columns-2 md:columns-3 lg:columns-4 gap-6 space-y-6">
                    @foreach($album->items as $item)
                        <div class="break-inside-avoid relative group rounded-sm sm:rounded-none overflow-hidden shadow-sm border border-charcoal-200 bg-charcoal-50 reveal-on-scroll reveal-up">
                            <button type="button" @click.prevent="openModal('{{ Storage::url($item->file_path) }}', @js($item->title ?? $album->title), @js($item->description ?? ''))" class="block h-full w-full text-left focus:outline-none">
                                @if(Storage::disk('public')->exists($item->file_path))
                                    ...
                                    <div class="absolute inset-0 bg-charcoal-950/0 group-hover:bg-charcoal-950/30 transition-colors duration-300 flex items-center justify-center pointer-events-none">
                                        <div class="w-12 h-12 bg-white/90 backdrop-blur-sm rounded-sm flex items-center justify-center opacity-0 group-hover:opacity-100 transform translate-y-4 group-hover:translate-y-0 transition-all duration-300 shadow-xl text-primary-600">
        ...
        <div x-show="isOpen" class="fixed inset-0 z-[200] flex items-center justify-center p-4 md:p-8 lg:p-12" style="display: none;">
            ...
            <div x-show="isOpen" 
                 ...
                 class="relative bg-white rounded-sm sm:rounded-none shadow-2xl overflow-hidden w-full max-w-6xl max-h-full flex flex-col md:flex-row z-10 border border-charcoal-200/20">
                
                <!-- Close Button -->
                <button @click="isOpen = false" class="absolute top-4 right-4 md:top-6 md:right-6 z-20 w-10 h-10 bg-white/50 hover:bg-white backdrop-blur-md rounded-sm flex items-center justify-center text-charcoal-900 transition-colors shadow-lg">
                ...
                <div class="w-full md:w-1/3 p-8 md:p-10 flex flex-col bg-white overflow-y-auto max-h-[40vh] md:max-h-[85vh] border-t md:border-t-0 md:border-l border-charcoal-100">
                    <div class="mb-auto">
                        <span class="inline-flex items-center py-1.5 px-3 bg-primary-50 text-primary-700 rounded-sm text-xs font-extrabold tracking-widest uppercase mb-5 border border-primary-100 shadow-sm">
