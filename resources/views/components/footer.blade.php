    ...
    <x-frontend.layout.container class="relative z-10">
        <div class="py-16 grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8">
            ...
            <div class="lg:col-span-4 lg:pr-12">
                <a href="{{ route('home') }}" class="flex items-center gap-3 mb-6 focus-ring rounded-xl py-1 px-1 -ml-1 group w-fit">
                    ...
                    <div class="flex flex-col">
                        <span class="font-black text-2xl text-white tracking-tighter leading-none">{{ $settings->get('site_name', 'Teknik Otomotif') }}</span>
            ...
            <div class="lg:col-span-8 grid grid-cols-2 sm:grid-cols-3 gap-8">
                ...
                <div>
                    ...
                    <ul class="space-y-4">
                        <li><a href="{{ route('home') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Beranda</a></li>
                        <li><a href="{{ route('about') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Tentang Jurusan</a></li>
                        <li><a href="{{ route('academic.programs') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Program Akademik</a></li>
                        <li><a href="{{ route('academic.teachers') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Direktori Guru</a></li>
                    </ul>
                </div>
                ...
                <div>
                    ...
                    <ul class="space-y-4">
                        <li><a href="{{ route('internships.index') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Info PKL</a></li>
                        <li><a href="{{ route('jobs.index') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Lowongan BKK</a></li>
                        <li><a href="{{ route('news.index') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Berita Terkini</a></li>
                        <li><a href="{{ route('download.index') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">Pusat Unduhan</a></li>
                    </ul>
                </div>
                ...
                <div class="col-span-2 sm:col-span-1">
                    ...
                    <ul class="space-y-4">
                        <li class="flex items-start gap-3 text-sm text-charcoal-400">
                            ...
                            <span>{{ $settings->get('contact_address', 'Jl. Pendidikan No. 1, Kota Belajar') }}</span>
                        </li>
                        <li class="flex items-center gap-3">
                            <svg class="h-5 w-5 shrink-0 text-charcoal-600" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"/></svg>
                            <a href="tel:{{ preg_replace('/[^0-9+]/', '', $settings->get('contact_phone', '(021) 123-4567')) }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">{{ $settings->get('contact_phone', '(021) 123-4567') }}</a>
                        </li>
                        <li class="flex items-center gap-3">
                            ...
                            <a href="mailto:{{ $settings->get('contact_email', 'info@otomotif.sch.id') }}" class="text-sm text-charcoal-400 hover:text-white transition-colors focus-ring rounded-md">{{ $settings->get('contact_email', 'info@otomotif.sch.id') }}</a>
                        </li>
                    </ul>
                </div>
            </div>
        ...
        <x-frontend.layout.container>
            <div class="py-6 flex flex-col md:flex-row items-center justify-between gap-4">
                <p class="text-sm text-charcoal-600 font-medium">
                    &copy; {{ date('Y') }} <span class="text-charcoal-300">{{ $settings->get('site_name', 'Teknik Otomotif') }}</span>. Seluruh Hak Cipta Dilindungi.
                </p>
