...
>
    ...
    >
        <!-- Modal Box -->
        <div class="mx-auto max-w-3xl transform divide-y divide-slate-100 overflow-hidden rounded-sm sm:rounded-none bg-white shadow-2xl ring-1 ring-black/5 transition-all" @click.outside="isSearchOpen = false">
            ...
            <form action="{{ route('search') ?? '/search' }}" method="GET" class="relative">
                ...
                <button type="button" @click="isSearchOpen = false" class="absolute right-3 top-3 p-1 rounded-sm text-slate-400 hover:text-red-500 hover:bg-red-50 focus:outline-none">
            ...
            <div x-show="searchQuery.length === 0" class="px-6 py-6 sm:px-8 bg-slate-50">
                ...
                <div class="flex flex-wrap gap-2">
                    <button type="button" @click="searchQuery = 'Prestasi'; $refs.searchInput.focus()" class="rounded-sm border border-slate-200 bg-white px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-slate-700 hover:bg-slate-50 hover:text-red-600 transition-colors">Prestasi</button>
                    <button type="button" @click="searchQuery = 'Fasilitas'; $refs.searchInput.focus()" class="rounded-sm border border-slate-200 bg-white px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-slate-700 hover:bg-slate-50 hover:text-red-600 transition-colors">Fasilitas Bengkel</button>
                    <button type="button" @click="searchQuery = 'Alumni'; $refs.searchInput.focus()" class="rounded-sm border border-slate-200 bg-white px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-slate-700 hover:bg-slate-50 hover:text-red-600 transition-colors">Data Alumni</button>
                    <button type="button" @click="searchQuery = 'PKL'; $refs.searchInput.focus()" class="rounded-sm border border-slate-200 bg-white px-3.5 py-1 text-xs font-bold uppercase tracking-wider text-slate-700 hover:bg-slate-50 hover:text-red-600 transition-colors">Lokasi PKL</button>
                </div>
            </div>
