    ...
    <div class="max-w-[1280px] mx-auto px-6 md:px-16">
        <div class="flex justify-between items-center transition-all duration-300 h-[64px]">
            ...
            <div class="hidden lg:flex lg:items-center lg:space-x-4 flex-grow justify-end">
                <a href="{{ route('home') }}" class="font-sans text-[14px] tracking-[-0.5px] uppercase transition-colors {{ request()->routeIs('home') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Beranda</a>
                
                <a href="{{ route('about') }}" class="font-sans text-[14px] tracking-[-0.5px] uppercase transition-colors {{ request()->routeIs('about') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Tentang</a>

                <a href="{{ route('academic.programs') }}" class="font-sans text-[14px] tracking-[-0.5px] uppercase transition-colors {{ request()->is('akademik*') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Akademik</a>

                <a href="{{ route('partnership.index') }}" class="font-sans text-[14px] tracking-[-0.5px] uppercase transition-colors {{ request()->is('pkl*') || request()->is('mitra-industri*') || request()->is('lowongan*') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Industri</a>
                
                <a href="{{ route('news.index') }}" class="font-sans text-[14px] tracking-[-0.5px] uppercase transition-colors {{ request()->is('berita*') || request()->is('pengumuman*') || request()->is('prestasi*') || request()->is('galeri*') || request()->is('alumni*') || request()->is('unduhan*') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Publikasi</a>
                
                <a href="{{ route('about') }}#contact" class="px-5 py-2 bg-figma-red text-white font-sans text-[14px] tracking-[-0.5px] uppercase rounded-[2px] hover:bg-figma-dark-red transition-colors focus-ring">Hubungi Kami</a>
