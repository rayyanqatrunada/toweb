<nav x-data="{ 
    ...
    <div class="max-w-[1280px] mx-auto px-6 md:px-16">
        <div class="flex justify-between items-center transition-all duration-300 h-[80px]">
            
            <!-- Logo Section -->
            <a href="{{ route('home') }}" class="flex-shrink-0 flex items-center gap-4 group focus-ring outline-none">
                <div class="font-heading font-extrabold text-[24px] text-figma-dark leading-none uppercase">
                    TEKNIK OTOMOTIF
                </div>
            </a>
            ...
            <div class="hidden lg:flex lg:items-center lg:space-x-6 flex-grow justify-end">
                <a href="{{ route('home') }}" class="font-sans text-[16px] tracking-[-0.8px] uppercase transition-colors {{ request()->routeIs('home') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Beranda</a>
                
                <a href="{{ route('about') }}" class="font-sans text-[16px] tracking-[-0.8px] uppercase transition-colors {{ request()->routeIs('about') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Tentang</a>

                <a href="{{ route('academic.programs') }}" class="font-sans text-[16px] tracking-[-0.8px] uppercase transition-colors {{ request()->is('akademik*') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Akademik</a>

                <a href="{{ route('partnership.index') }}" class="font-sans text-[16px] tracking-[-0.8px] uppercase transition-colors {{ request()->is('pkl*') || request()->is('mitra-industri*') || request()->is('lowongan*') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Industri</a>
                
                <a href="{{ route('news.index') }}" class="font-sans text-[16px] tracking-[-0.8px] uppercase transition-colors {{ request()->is('berita*') || request()->is('pengumuman*') || request()->is('prestasi*') || request()->is('galeri*') || request()->is('alumni*') || request()->is('unduhan*') ? 'text-figma-dark font-bold' : 'text-figma-gray hover:text-figma-dark' }}">Publikasi</a>
                
                <a href="{{ route('about') }}#contact" class="px-6 py-3 bg-figma-red text-white font-sans text-[16px] tracking-[-0.8px] uppercase rounded-[2px] hover:bg-figma-dark-red transition-colors focus-ring">Hubungi Kami</a>
                
                <button type="button" @click="$dispatch('open-search')" aria-label="Search" class="text-figma-gray hover:text-figma-dark transition-colors focus-ring p-1 ml-4">
    ...
    <div x-show="mobileMenuOpen" 
        ...
        <div class="flex-1 overflow-y-auto px-6 py-6 space-y-6">
            ...
            <a href="{{ route('about') }}#contact" class="block text-center mt-8 px-6 py-3 bg-figma-red text-white font-sans text-lg uppercase rounded-[2px] hover:bg-figma-dark-red transition-colors">Hubungi Kami</a>
        </div>
    </div>
</nav>
