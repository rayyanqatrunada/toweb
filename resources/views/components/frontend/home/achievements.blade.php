    ...
    <div class="max-w-[1280px] mx-auto px-6 md:px-16">
        ...
        <div class="flex flex-col lg:flex-row gap-16 lg:gap-24">
            ...
            <div class="w-full lg:w-5/12 flex flex-col items-start reveal-on-scroll reveal-up">
                ...
                <a href="{{ route('achievements.index') }}" class="group flex items-center gap-4 text-figma-dark hover:text-figma-red transition-colors font-sans font-bold text-[16px] uppercase tracking-[-0.5px]">
                    <span class="relative">
                            ...
                            <div class="relative group">
                                ...
                                <div class="flex flex-col bg-charcoal-50 p-6 md:p-8 hover:bg-white hover:shadow-xl transition-all duration-300 border border-transparent hover:border-gray-200 -mt-2">
                                    <div class="flex items-center gap-4 mb-3">
                                        <span class="px-3 py-1 bg-figma-dark text-white font-heading font-bold text-[14px]">{{ $achievement->date ? $achievement->date->format('Y') : '' }}</span>
                                        <span class="font-sans font-bold text-[14px] text-figma-red uppercase tracking-wide">{{ $achievement->rank }}</span>
                                    </div>
                                    <h3 class="font-heading font-bold text-[22px] md:text-[28px] text-figma-dark leading-tight mb-2 group-hover:text-figma-red transition-colors">{{ $achievement->title }}</h3>
                                    <p class="font-sans text-[15px] text-gray-500">{{ $achievement->organizer }}</p>
                                </div>
                            </div>
                        @endforeach
                    </div>
