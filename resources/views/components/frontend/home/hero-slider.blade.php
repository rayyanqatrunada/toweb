...
<section class="relative w-full h-[680px] lg:h-[780px] bg-charcoal-900 overflow-hidden" data-hero-slider aria-label="Hero Image Slider">
    ...
    <div class="relative w-full h-full">
        @foreach($slides as $index => $slide)
            <div class="hero-slide absolute inset-0 w-full h-full transition-opacity duration-700 ease-in-out {{ $index === 0 ? 'opacity-100 z-10' : 'opacity-0 z-0' }}" aria-hidden="{{ $index === 0 ? 'false' : 'true' }}">
                
                @if($index === 0)
                    <!-- Eager load first image -->
                    <img src="{{ $slide['image'] }}" alt="Slide {{ $index + 1 }}" loading="eager" fetchpriority="high" class="absolute inset-0 w-full h-full object-cover">
                @else
                    <!-- Lazy load subsequent images with data-src for JS -->
                    <img data-src="{{ $slide['image'] }}" src="data:image/gif;base64,R0lGODlhAQABAAD/ACwAAAAAAQABAAACADs=" alt="Slide {{ $index + 1 }}" class="absolute inset-0 w-full h-full object-cover">
                @endif
                ...
                <div class="absolute inset-0 z-30 flex items-center">
                    <div class="w-full max-w-[1280px] mx-auto px-6 md:px-16 flex flex-col justify-center">
                        <div class="max-w-[720px] {{ $index === 0 ? 'reveal-on-scroll reveal-up' : '' }}">
                            <!-- Eyebrow -->
                            ...
                            <div class="flex flex-col sm:flex-row items-center gap-4">
                                <a href="{{ route('about') }}" class="flex justify-center items-center px-8 py-4 bg-figma-red text-white font-sans font-bold text-[16px] tracking-[-0.5px] uppercase rounded-[2px] w-full sm:w-auto h-[56px] hover:bg-figma-dark-red transition-all duration-300 focus-ring shadow-lg shadow-figma-red/20 group">
                                    Jelajahi TBSM
    ...
    <div class="absolute bottom-0 left-0 w-full z-40 bg-gradient-to-t from-charcoal-950 to-transparent pt-12 pb-8">
        <div class="max-w-[1280px] mx-auto px-6 md:px-16 flex items-center justify-between">
            ...
            <div class="flex items-center gap-3">
                @foreach($slides as $index => $slide)
                    <button type="button" aria-label="Go to slide {{ $index + 1 }}" class="hero-dot h-2 rounded-full transition-all duration-300 focus-ring {{ $index === 0 ? 'bg-figma-red w-8' : 'bg-white/50 w-2 hover:bg-white' }}"></button>
                @endforeach
            </div>
            ...
            <div class="flex items-center gap-6">
                <div class="hidden md:flex items-baseline gap-1 font-heading text-white">
                    ...
                    <span class="text-[14px] text-gray-500 font-normal">/ 0{{ count($slides) }}</span>
                </div>
