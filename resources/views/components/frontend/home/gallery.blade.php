        ...
        @if($galleries && $galleries->count() > 0)
            <div class="columns-1 md:columns-2 lg:columns-3 gap-6 space-y-6 reveal-on-scroll reveal-up delay-100">
                @foreach($galleries as $gallery)
                    <a href="{{ route('gallery.show', $gallery->slug) }}" class="block relative group overflow-hidden break-inside-avoid shadow-sm hover:shadow-xl transition-all duration-300 bg-white">
                        ...
                        <div class="absolute bottom-0 left-0 w-full p-6 translate-y-4 opacity-0 group-hover:translate-y-0 group-hover:opacity-100 transition-all duration-300">
                            <h3 class="font-heading font-bold text-[18px] text-white mb-1">{{ $gallery->title }}</h3>
            ...
            <div class="mt-12 md:mt-16 text-center">
                <a href="{{ route('gallery.index') }}" class="inline-flex items-center justify-center px-8 py-4 border-2 border-figma-dark text-figma-dark font-sans font-bold text-[14px] uppercase tracking-wide hover:bg-figma-dark hover:text-white transition-colors focus-ring">
