        ...
        <div style="background-color: #ffffff !important; border: 1px solid #e2e8f0; border-radius: 24px; box-shadow: 0 -12px 35px rgba(0, 0, 0, 0.2); padding: 16px 14px 18px 14px; overflow: hidden;">
            ...
            <div style="display: grid; grid-template-columns: repeat(4, 1fr); gap: 8px; max-height: 60vh; overflow-y: auto;">
                @foreach($menuTiles as $item)
                    <a href="{{ $item['route'] }}" 
                       onclick="window.closeMobileNavDrawer()"
                       style="display: flex; flex-direction: column; align-items: center; justify-content: center; padding: 8px 4px; border-radius: 14px; text-decoration: none; transition: background-color 0.15s; {{ $item['active'] ? 'background-color: #fef2f2; border: 1px solid #fecaca;' : 'background-color: #f8fafc; border: 1px solid #f1f5f9;' }}">
                       
                        <!-- Kotak Icon Persegi -->
                        <div style="position: relative; width: 44px; height: 44px; border-radius: 12px; display: flex; align-items: center; justify-content: center; {{ $item['active'] ? 'background-color: #dc2626; color: #ffffff; box-shadow: 0 4px 12px rgba(220, 38, 38, 0.3);' : 'background-color: #ffffff; color: #334155; border: 1px solid #e2e8f0;' }}">
                            
                            @if($item['icon'] === 'home')
                                <svg style="width: 20px; height: 20px;" fill="{{ $item['active'] ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
                        ...
                        <!-- Label Nama Menu -->
                        <span style="font-family: system-ui, sans-serif; font-size: 11px; font-weight: 700; text-align: center; margin-top: 5px; line-height: 1.1; overflow: hidden; text-overflow: ellipsis; white-space: nowrap; max-width: 100%; {{ $item['active'] ? 'color: #dc2626;' : 'color: #334155;' }}">
                            {{ $item['label'] }}
                        </span>
                    </a>
                @endforeach
    ...
    <nav aria-label="Mobile Navigation Bar" 
        ...
        <div style="max-width: 540px; margin: 0 auto; padding: 0 8px; display: flex; align-items: center; justify-content: space-around; height: 100%; background: #ffffff !important;">
            ...
            <a href="{{ route('academic.programs') }}" 
               onclick="window.closeMobileNavDrawer()"
               style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; text-decoration: none; color: {{ $isPrograms ? '#dc2626' : '#64748b' }};">
                <div style="position: relative; display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 9999px; {{ $isPrograms ? 'background-color: #fef2f2;' : '' }}">
                    <svg style="width: 20px; height: 20px;" fill="{{ $isPrograms ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('academic.teachers') }}" 
               onclick="window.closeMobileNavDrawer()"
               style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; text-decoration: none; color: {{ $isTeachers ? '#dc2626' : '#64748b' }};">
                <div style="position: relative; display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 9999px; {{ $isTeachers ? 'background-color: #fef2f2;' : '' }}">
                    <svg style="width: 20px; height: 20px;" fill="{{ $isTeachers ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
            ...
            <a href="{{ route('academic.facilities') }}" 
               onclick="window.closeMobileNavDrawer()"
               style="flex: 1; display: flex; flex-direction: column; align-items: center; justify-content: center; text-decoration: none; color: {{ $isFacilities ? '#dc2626' : '#64748b' }};">
                <div style="position: relative; display: flex; align-items: center; justify-content: center; width: 32px; height: 32px; border-radius: 9999px; {{ $isFacilities ? 'background-color: #fef2f2;' : '' }}">
                    <svg style="width: 20px; height: 20px;" fill="{{ $isFacilities ? 'currentColor' : 'none' }}" stroke="currentColor" viewBox="0 0 24 24">
