<nav x-data="{ mobileMenuOpen: false }" x-effect="document.body.style.overflow = mobileMenuOpen ? 'hidden' : ''" class="sticky top-0 z-50 bg-white/90 backdrop-blur-md border-b border-slate-200 shadow-sm transition-all duration-300">
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="flex justify-between items-center h-16 md:h-18">
            <!-- Logo Section -->
            <a href="<?php echo e(route('home') ?? '/'); ?>" class="flex-shrink-0 flex items-center gap-3">
                ...
                <span class="font-bold text-xl text-slate-900 tracking-tight hidden sm:block"><?php echo e($settings->get('site_name', 'Teknik Otomotif')); ?></span>
            </a>

            <!-- Desktop Menu -->
            <div class="hidden lg:flex md:items-center md:space-x-8">
                <a href="<?php echo e(route('home') ?? '/'); ?>" class="px-1 pt-1 border-b-2 text-sm font-medium transition duration-150 <?php echo e(request()->routeIs('home') ? 'border-red-600 text-red-600' : 'border-transparent text-slate-600 hover:text-slate-900 hover:border-slate-300'); ?>">Beranda</a>
                ...
                <div x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false" class="relative inline-block text-left">
                    ...
                    <div x-show="open" x-transition class="absolute z-50 left-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5 focus:outline-none" style="display: none;">
                        <div class="py-1">
                            <a href="<?php echo e(route('about') ?? '/profil'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Tentang Jurusan</a>
                            <a href="<?php echo e(route('academic.teachers') ?? '/guru'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Guru & Staf</a>
                            <a href="<?php echo e(route('academic.facilities') ?? '/fasilitas'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Fasilitas</a>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false" class="relative inline-block text-left">
                    ...
                    <div x-show="open" x-transition class="absolute z-50 left-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5" style="display: none;">
                        <div class="py-1">
                            <a href="<?php echo e(route('academic.programs') ?? '/akademik/program'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Program & Kompetensi</a>
                            <a href="<?php echo e(url('/akademik/kurikulum')); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Kurikulum</a>
                        </div>
                    </div>
                </div>
                ...
                <div x-data="{ open: false }" @click.outside="open = false" @keydown.escape.window="open = false" class="relative inline-block text-left">
                    ...
                    <div x-show="open" x-transition class="absolute z-50 left-0 mt-2 w-48 rounded-md shadow-lg bg-white ring-1 ring-black ring-opacity-5" style="display: none;">
                        <div class="py-1">
                            <a href="<?php echo e(route('internships.index') ?? '/pkl'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Praktik Kerja Lapangan</a>
                            <a href="<?php echo e(route('partnership.index') ?? '/kemitraan'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Mitra Industri</a>
                            <a href="<?php echo e(route('jobs.index') ?? '/bursa-kerja'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Lowongan Kerja</a>
                            <a href="<?php echo e(route('alumni.index') ?? '/alumni'); ?>" class="block px-4 py-2 text-sm text-slate-700 hover:bg-slate-100 hover:text-red-600">Alumni</a>
                        </div>
                    </div>
                </div>
            </div>
    ...
    <div id="mobile-menu" x-show="mobileMenuOpen" x-transition class="lg:hidden bg-white border-t border-slate-100 shadow-xl" style="display: none;">
        <div class="px-4 pt-2 pb-6 space-y-1 overflow-y-auto max-h-[calc(100vh-64px)]">
            <a href="<?php echo e(route('home') ?? '/'); ?>" class="block px-3 py-3 rounded-md text-base font-semibold text-red-600 bg-red-50">Beranda</a>
            ...
            <div x-data="{ expanded: false }">
                ...
                <div x-show="expanded" class="pl-6 pr-3 py-2 space-y-2 bg-slate-50/50 rounded-md">
                    <a href="<?php echo e(route('about') ?? '/profil'); ?>" class="block text-sm text-slate-600 hover:text-red-600 py-2">Tentang Kami</a>
                    <a href="<?php echo e(route('academic.teachers') ?? '/guru'); ?>" class="block text-sm text-slate-600 hover:text-red-600 py-2">Guru & Staf</a>
                    <a href="<?php echo e(route('academic.facilities') ?? '/fasilitas'); ?>" class="block text-sm text-slate-600 hover:text-red-600 py-2">Fasilitas</a>
                </div>
            </div>
...
<?php endif; ?>
<?php /**PATH D:\Project\test\Web Jurusan\TOWEB\resources\views\components\navbar.blade.php ENDPATH**/ ?>